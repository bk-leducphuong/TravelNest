...
export default {
  ...
  computed: {
    ...mapGetters('auth', ['isOtpVerified'])
  },
  methods: {
    ...mapActions('auth', ['verifyOtp']),
    async moveToNext(input, index) {
      ...
      if (index === 3 && this.otp.join('').length === 4) {
        await this.verifyOtp({ phoneNumber: this.phoneNumber, otp: this.otp.join('') })
        if (this.isOtpVerified) {
          this.$emit('update-is-verified', true)
        } else {
          this.numberOfAttempts++
          this.toast.error('Invalid OTP! Please try again.')
          if (this.numberOfAttempts === 3) {
            this.toast.error('OTP verification failed! Please try again later.')
            this.$emit('update-is-verified', false)
          }
        }
      } else if (index < 3) {
...
<template>
  <div class="validation-container">
    <div class="validation-card">
      ...
      <p class="resend-text">
        Didn't receive code?
        <a href="#" class="resend-link" @click="resendOtp">Resend</a>
      </p>
    </div>
  </div>
</template>
